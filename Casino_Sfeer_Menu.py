...
try:
    # PyInstaller --onefile extracts bundled data (assets/) to sys._MEIPASS at runtime,
    # not next to the exe, so the base path must account for that when frozen.
    _base_path = getattr(sys, "_MEIPASS", os.path.dirname(os.path.abspath(__file__)))
    icon_path = os.path.join(_base_path, "Assets", "logo.jpg")
